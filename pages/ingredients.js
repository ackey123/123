...
export default function Ingredients(){
  const [ings, setIngs] = useState([]); // {name, soonest}

  useEffect(()=>{
    const items = loadItems();
    const m = new Map(); // ing -> 最短残日数
    ...
    const arr = Array.from(m.entries()).map(([name, soonest])=>({name, soonest}))
  ...
  return (
    <main style={{padding:24, fontFamily:'sans-serif'}}>
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
        ...
        )) : <small>（まだありません。/upload で追加してください）</small>}
      </div>
    </main>
  );
}
